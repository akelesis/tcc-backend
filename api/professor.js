...
module.exports = app => {
    ...
    const post = async (req, res) => {
        ...
        try {
            existsOrError(professor.professor_name, "nome indefinido")
            existsOrError(professor.department, "departamento não informado")
            
            const professorFromDB = await app.db('professor').where({ professor_name: professor.professor_name, deleted_at: null  }).first()
            ...
            const professorSaved = await app.db('professor')
                .insert({ professor_name: professor.professor_name, department: professor.department, email: professor.email, created_at: new Date().toISOString().replace('Z', '').replace('T', ' ') })
    ...
    const put = async (req, res) => {
        ...
        try {
            const updatedProfessor = await app.db('professor')
                .update({ name: professor.professor_name, department: professor.department, email: professor.email, updated_at: new Date().toISOString().replace('Z', '').replace('T', ' ') })
